function trials = generate_trials_experiment_with_noStop(n)
%Column 
% 1: cue (0 = uninformative, 1 = left, 2 = right, 3 = noStop)
...
conds = [0 0 1 2 3]; % cued stop side (0 for non-informative, 1 for left, 2 for right, 3 for no-stop, 4 for both). Also represents ratio
go_stop = [7 3;7 3;7 3;7 3; 10 0]; %indicates the relative ratio of go_stop trials. Number indicates number of trials in single set. ([1 1] will give same as [5 5])
stop_signal = [1 2 1 2 0]; %actual stop signal IN CASE a stop signal is given that trial. Same length as conds. left: 1; right: 2; none: 0, 4:center
iti = [2000 2000]; %uniform range of itis
anticipation = [1000 1000 2000 3000 3000];
...
trials(:,6) = repmat(anticipation,1,n/size(anticipation,2))';
trials(:,6) = randswap(trials(:,6),1);
